            ...
            <td>
              <div class="actions">
                <button
                  ...
                  @click="openEditModal(t.tournament.id)"
                >
                ...
                <button
                  ...
                  @click="requestDelete(t.tournament.id)"
                >
...
//Services
import { getTeamsByTournamentId, deleteTournament } from '@/services/tournamentService';
...
onMounted(async () => {
  try {
    tournamentsAdminDTO.value = await getTeamsByTournamentId();
  } catch (error) {
...
async function confirmDelete() {
  if (pendingDeleteId.value !== null) {
    try {
      await deleteTournament(pendingDeleteId.value);
      tournamentsAdminDTO.value = tournamentsAdminDTO.value.filter(
        (t) => t.tournament.id !== pendingDeleteId.value
      );
      openPopup('success', 'Torneo eliminado correctamente');
    } catch (error: any) {
      if (error.response?.status === 404) {
        openPopup('alert', 'El torneo no existe');
      } else if (error.response?.status === 400) {
        openPopup('error', 'Solicitud inválida');
      } else {
        openPopup('error', 'Error al eliminar el torneo');
      }
...
<style scoped>
/* Estilos del overlay de confirmación */
.confirm-overlay {
  position: fixed;
  top: 0;
  left: 0;
