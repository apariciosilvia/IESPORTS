...
const personNameAdd = ref<string>('')
const personEmailAdd = ref<string>('')
...
async function savePerson() {
  console.log('usuario para crear', personNameAdd.value, personEmailAdd.value, selectedRoleId.value, selectedCourseId.value)
  const personRegisterByAdminDTO: PersonRegisterByAdminDTO = {
    name: personNameAdd.value,
    email: personEmailAdd.value,
    roleId: selectedRoleId.value as number,
    courseId: selectedCourseId.value as number
  }
  try {
    console.log('DTO', personRegisterByAdminDTO)
    await registerAUser(personRegisterByAdminDTO)
    emit('updated','Persona creada correctamente')
    openPopup('success','Persona creada')
